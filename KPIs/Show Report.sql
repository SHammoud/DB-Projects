SELECT DD.date                                                     AS 'Show Date',
      ...
      IF(D.cancelled=1,'YES','NO')                                  AS 'Deal Cancelled',
       ...
       ROUND(IF(D.cancelled=1,ROUND(IFNULL(DCF.fee,0), 2),ROUND(DD.fee+IFNULL(OV.amount,0) / 100, 2)) / COALESCE(DD.exchangeRate,CX.rate,1),2) AS 'Show Commissionable Gross GBP'
...
WHERE D.firstDate BETWEEN '2012-01-01' AND '2023-12-31'
